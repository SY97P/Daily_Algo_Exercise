...
def bfs(queue, visited, matrix, weight) : 
	while queue :
		...
		if matrix[i][j] != 0 and weight > matrix[i][j] : 
			...
			for q in queue : 
				qi, qj, qc = q
				if matrix[qi][qj] != 0 and matrix[qi][qj] < weight and qc <= count :
					temp.append(q)
			temp.sort()
			print(temp)
			return temp[0]
...
def dfs(i, j, matrix, fishes, weight, prey) : 
	print("i, j, fishes, weight, prey : ", i, j,fishes,weight,prey)
	...
	if no_prey :
		...
		return_Value = bfs(deque([(i, j, 0)]), visited, matrix, weight)
...
for _ in range(8) : 
	...
	print("n, fishes, weight, answer : ", n, fishes, weight, answer)
